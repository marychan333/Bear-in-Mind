...
class Login
{
	...
	{
		//数据库验证
		$sql = "SELECT username,id FROM users WHERE username = '".$this->username."' and password = '".$this->password."'";
		...
		if (!$result) {
			...
			echo "<script>alert('Login Success!');location.href = '../todo.php'</script>";
			exit();
		}
	}
